...
class ConformerCls(nn.Module):
    def __init__(
        ...
        self.agg = nn.Conv1d(
            in_channels=24,
            out_channels=24 * num_lstm_hidden,
            kernel_size=num_lstm_hidden,
        )
        self.classifier = nn.Linear(
            in_features=24 * num_lstm_hidden, out_features=num_classes, bias=True
        )
        # self.classifier = nn.Conv1d(
        #     in_channels=24, out_channels=num_classes, kernel_size=num_lstm_hidden
        # )
    ...
    def forward(self, x: torch.Tensor, input_lengths: torch.Tensor) -> torch.Tensor:
        ...
        x = self.agg(x)
        logits = self.classifier(x.squeeze(2))
        return logits
...
if __name__ == "__main__":
    ...
    console.log(output)
    # console.log(conformer_classifier)
    # console.log(pred)
