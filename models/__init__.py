...
class ARB_Add(nn.Module):
    def __init__(
        ...
        self.aug = aug
        if self.aug:
            self.attention_aug = aug_block(
                out_channels * 4, out_channels * 4, kernel_size, dk, dv, Nh, shape
            )
    ...
    def forward(self, inputs):
        if not (self.kernel_size % 2):
            inputs = torch.nn.functional.pad(
                inputs, (0, 1, 0, 1), mode="constant", value=0
            )
        x = self.conv1(inputs)
        if self.aug:
